...
## Adding another mock community

1. Add a row to `datasets.tsv` with its libraries, provider and `source_id`.
2. If it needs a new reference set, add a builder to `REFERENCE_SETS` in
   `1_get_inputs.py` that writes `reference_genomes.fasta` and
   `reference_map.tsv` (`reference_id`, `molecule_label`, `agent_label`).
3. `bash 0_check_environment.sh`, then `bash run_all.sh --dataset <name>`.

A reference set is ground truth, so it has to be *verified*, not guessed. The
phage genomes are fetched from NCBI by name and accepted only when the length
and GC match the published table — names like `PHAGE1`, `KUW1` and `J1` are not
unique in NCBI, and an unchecked name search would quietly mislabel contigs.

What that check turned up, on this mock alone:

- Three genomes are deposited under a different name than Table S1 uses —
  `KUW1` is *Paracoccus phage ParKuw1* (OQ376857), `PARMAL1` is *Paracoccus
  phage ParMal1* (OQ376858), and `SM032` is *vB_VpaS_sm030* (OV032860), from
  the same submission as sm033 and at exactly the published 79,660 bp.
- `SLUR29` is accepted at 48,466 bp against a published 48,593 bp, which is why
  the length tolerance is 1 % rather than exact.
- **`J1` and `J2` resolve to the same record**, LR027385.1, whose own title
  reads *"Escherichia virus vB_Eco_mar001J1 strain vB_Eco_mar002J2"*. NCBI holds
  them as one genome, which independently confirms Table S2's 100 % identity.
  Both labels are kept in the reference set, and phase 3 merges them into the
  single truth genome `J1|J2` on the evidence rather than on this note.
- Candidates are ranked by exact length and by whether the record's title
  carries the phage's own name, because relatives in this mock differ by less
  than the tolerance: *vB_Eco_Sip* at 50,809 bp would otherwise have been
  accepted as `SWAN`, whose real record is LT841304.1 at 50,865 bp.

### Considered and not used

**VirMock1** (BioProject PRJNA1348896, 8 viruses spiked into stool/saliva/OP
wash/BAL, 323 Illumina runs). Public and easy to fetch, but smaller than the
phage mock on every axis that matters here — 8 genomes, no deliberate
microdiversity — and its run names are bare numeric IDs, so mapping runs to
sample matrices needs Table S14 of the paper. Worth revisiting if a
mock-in-real-background test is wanted, which is the one thing it has and
neither dataset here does.
